...
public class SR {
    ...
    public static void ServerSocket(String filename) throws IOException, SintaxeIncorretaException, ClassNotFoundException, InterruptedException {
        ...
        logfile.updateLogFileEV("log-file-read", LocalDateTime.now(), "EV", "/var/dns/tokyoSR.log");
        logfile.updateLogFileEV("conf-file-read", LocalDateTime.now(), "EV", "tokyoSR.conf");
        LocalDateTime timeRR = LocalDateTime.now();
        int tamanho = 0;

        while(running) {
            logfile.updateLogFileST(4443, "debug", 5000, LocalDateTime.now(), "ST",  "127.0.0.1");
            while(connection) {
                try {
                    socket = new Socket(host.getHostName(), 4443);
            ...
            oos.writeObject(parserConfig.getWorkingDomain());//SR envia o domínio em que trabalha
            ...
            if (i < 65535) {
                oos = new ObjectOutputStream(socket.getOutputStream());
                oos.writeObject("Aceito");//SR aceita receber o n de linhas
        ...
        logfile.updateLogFileZT("SR", "4443", duracao.toMillis(), tamanho, timeZT, "ZT");
        socket.close();
    }
